    ...
    try {
      ...
      if (trade_type == 1) {
        ...
        tradeData = await sails.sendNativeQuery("Select jst_trade_history.id,jst_trade_history.user_id,jst_tr" +
          ...
          " users.email, jst_trade_history.created_at, jst_trade_history.execution_report,jst_trade_history.faldax_fees, jst_trade_history.network_fees ,jst_trade_history.order_id" + query, [])
        ...
        if ((data && data != "")) {
          if (data && data != "" && data != null) {
            // whereAppended = true;
            query += ' AND'
            query += " (LOWER(users.email) LIKE '%" + data.toLowerCase() + "%' OR LOWER(simplex_trade_history.symbol) LIKE '%" + data.toLowerCase() + "%' OR simplex_trade_history.payment_id LIKE '%" + data + "%' OR simplex_trade_history.quote_id LIKE '%" + data + "%' OR simplex_trade_history.address LIKE '%" + (data).trim() + "%'";
            if (!isNaN(data)) {
