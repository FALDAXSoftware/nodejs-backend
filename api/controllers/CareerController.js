...
module.exports = {
  applyJob: async function (req, res) {
    ...
    if (jobDetail) {
        ...
        .upload(async function (err, uploadedDoc) {
          try {
            if (uploadedDoc.length > 0) {
              ...
              return res.json({"status": 400, "message": "Resume is not present."});
            }
          } catch (e) {
            throw e;
          }
      ...
      return res.json({"status": 400, "message": "Job id is not valid."});
    }
  },
  ...
  getAllJobsCMS: async function (req, res) {
    ...
    let careerDesc = await Statics.findOne({slug: 'career'});
    if (allJobs) {
      return res.json({"status": 200, "message": "All jobs retrived successfully", "data": allJobs, careerDesc, allJobsCount});
    } else {
      return res
        .status(500)
        .json({
  ...
  getJobDetail: async function (req, res) {
    try {
      ...
      if (jobDetail) {
        return res.json({"status": 200, "message": "Job details retrived successfully", "data": jobDetail});
      } else {
        return res
          .status(400)
          .json({status: 400, "err": "Job does not exists"});
      }
    } catch (err) {
      return res
        .status(500)
  ...
  editJob: async function (req, res) {
    ...
    if (job) {
      ...
      return res
        .status(400)
        .json({status: 400, "err": "Job not found"});
    }
  },
  ...
  deleteJob: async function (req, res) {
    try {
      ...
      if (!job_id) {
        res
          .status(500)
          .json({"status": 500, "err": "Job id is not sent"});
        return;
      }
  ...
  getJobApplications: async function (req, res) {
    try {
      ...
      let query = " from job_applications";
      query += " WHERE job_id = " + job_id;
      if ((data && data != "")) {
        if (data && data != "" && data != null) {
          query = query + " AND LOWER(first_name) LIKE '%" + data.toLowerCase() + "%'OR LOWER(last_name) LIKE '%" + data.toLowerCase() + "%'OR LOWER(linkedin_profile) LIKE '%" + data.toLowerCase() + "%'OR LOWER(email) LIKE '%" + data.toLowerCase() + "OR LOWER(website_url) LIKE '%" + data.toLowerCase() + "%'";
          if (!isNaN(data)) {
      ...
      console.log('query job applications', query)
      let jobApplicationsData = await sails.sendNativeQuery("Select *" + query, [])

      jobApplicationsData = jobApplicationsData.rows;

      let jobApplicationsDataCount = await sails.sendNativeQuery("Select COUNT(id)" + countQuery, [])
      jobApplicationsCount = jobApplicationsDataCount.rows[0].count;
      if (jobApplicationsData) {
        return res.json({"status": 200, "message": "All job applications retrived successfully", "data": jobApplicationsData, jobApplicationsCount});
      }
