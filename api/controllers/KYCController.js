...
module.exports = {
  ...
  getAllKYCData: async function (req, res) {
    ...
    if (data) {
      ...
      // KYC.query("select * from kyc where steps=3 AND status='true' AND
      // ((direct_response != 'ACCEPT' AND webhook_response != 'ACCEPT') OR (id_type =
      // '4' AND direct_response != 'ACCEPT'))", [], function (err, rawResult) {
      // console.log(rawResult); });
      var rawResult = await sails.sendNativeQuery("SELECT * FROM public.kyc ORDER BY updated_at LIMIT $1 OFFSET $2", [
        limit, limit * (page - 1)
      ]);
