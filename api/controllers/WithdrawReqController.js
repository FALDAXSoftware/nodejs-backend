...
module.exports = {
    ...
    getAllWithdrawReq: async function (req, res) {
        ...
        if ((data && data != "")) {
            if (data && data != "" && data != null) {
                ...
                query += " (LOWER(users.email) LIKE '%" + data.toLowerCase() + "%'" +
                    " OR LOWER(withdraw_request.source_address) LIKE '%" + data.toLowerCase() + "%'" +
                    " OR LOWER(withdraw_request.destination_address) LIKE '%" + data.toLowerCase() + "%'";
                if (!isNaN(data)) {
        ...
        if (start_date && end_date) {
            ...
            query += " withdraw_request.created_at >= '" + await sails.helpers.dateFormat(start_date) + " 00:00:00" +
                "' AND withdraw_request.created_at <= '" + await sails.helpers.dateFormat(end_date) + " 23:59:59'";
        }
